...
export const updateProduct = async (id, access_token, data) => {
  try {
    ...
    console.error('Lỗi khi refresh_token: ', error.response ? error.response.data : error)
    return null
  }
}
export const deleteProduct = async (id, access_token) => {
  try {
    ...
    console.log('Sản phẩm đã được xoá: ', res)
    return res.data
  } catch (error) {
    console.error('Lỗi khi xoá sản phẩm: ', error.response ? error.response.data : error)
    return null
  }
}
